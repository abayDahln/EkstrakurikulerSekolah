...
// Create Schedule Modal yang sudah diperbaiki
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			if (response.ok) {
				...
				setError(result.message || "Gagal membuat jadwal");
			}
		} catch (error) {
					...
					<div>
						...
						{ekskulList.length === 0 && (
							<p className={`text-sm mt-2 ${
								...
								Anda belum memiliki ekstrakurikuler yang dikelola. 
								Hubungi administrator untuk ditambahkan sebagai pembina.
							</p>
						)}
						{ekskulList.length > 0 && (
							<p className={`text-sm mt-2 ${
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				...
				// PERBAIKAN: Gunakan endpoint dashboard untuk mendapatkan daftar ekstrakurikuler
				const dashboardResponse = await fetch(`${API_URL}/api/pembina/dashboard`, {
					headers: { Authorization: `Bearer ${token}` },
				});

				if (dashboardResponse.ok) {
					const dashboardResult = await dashboardResponse.json();
					if (isMounted && dashboardResult.status === 200) {
						// Ambil data dari response dashboard
						setEkskulList(dashboardResult.data?.ekskulList || []);
					}
				} else {
					// Fallback: coba endpoint my-extracurricular jika dashboard gagal
