...
// Skeleton Component yang sesuai dengan layout
const SkeletonProfile = ({ darkMode }) => (
  <div className="space-y-6">
    {/* Header Skeleton */}
    <div className={`rounded-2xl shadow-lg p-8 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
      <div className="flex flex-col items-center text-center">
        <div className={`w-40 h-40 rounded-full animate-pulse mx-auto mb-4 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className={`h-8 w-64 rounded animate-pulse mx-auto mb-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className={`h-6 w-80 rounded animate-pulse mx-auto mb-3 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className={`h-10 w-32 rounded-full animate-pulse mx-auto ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className={`h-4 w-48 rounded animate-pulse mx-auto mt-6 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
      </div>
    </div>

    {/* Info & Stats Skeleton */}
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {/* Left Card - Personal Info Skeleton */}
        ...
        <div className="space-y-5">
          {[...Array(3)].map((_, index) => (
            <div key={index} className="flex items-start gap-3">
              <div className={`w-10 h-10 rounded-lg animate-pulse flex-shrink-0 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
              <div className="flex-1 space-y-2">
                <div className={`h-4 w-24 rounded animate-pulse ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
      ...
      {/* Right Card - Statistics Skeleton */}
      <div className={`rounded-2xl shadow-lg p-6 h-full ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
        <div className={`h-7 w-48 rounded animate-pulse mb-6 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className="grid grid-cols-2 gap-4">
          {[...Array(4)].map((_, index) => (
            <div key={index} className={`p-4 rounded-xl ${darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
              <div className={`w-12 h-12 rounded-lg animate-pulse mb-3 ${darkMode ? 'bg-slate-600' : 'bg-slate-200'}`} />
    ...
    <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {[...Array(3)].map((_, index) => (
          <div key={index} className={`rounded-xl overflow-hidden ${darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
            <div className={`w-full h-40 animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-200'}`} />
            <div className="p-4 space-y-3">
              <div className={`h-6 w-32 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              <div className={`h-4 w-full rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              <div className={`h-4 w-3/4 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              <div className="flex gap-2">
                <div className={`h-6 w-20 rounded-full animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
                <div className={`h-6 w-16 rounded-full animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              </div>
            </div>
          </div>
        ))}
                  ...
                  <div className={`rounded-2xl shadow-lg p-8 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
                    <div className="flex flex-col items-center text-center">
                      <img
                        ...
                        onError={(e) => {
                          e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(profileData.name)}&background=0ea5e9&color=fff&size=160`;
                        }}
                      />
                      ...
                      {(isPembina ? profileData.managedExtracurriculars : profileData.extracurriculars)?.map((ekskul) => (
                        ...
                        >
                          <img
                            ...
                            onError={(e) => {
                              e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(ekskul.name)}&background=0ea5e9&color=fff&size=400x200`;
                            }}
                          />
                          <div className="p-4">
