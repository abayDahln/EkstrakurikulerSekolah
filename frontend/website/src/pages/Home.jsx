...
const Home = ({ darkMode }) => {
	...
	const fetchAllData = async () => {
		try {
			...
			if (scheduleResult.status === 200 && Array.isArray(scheduleResult.data)) {
				const localSchedules = scheduleResult.data.map((item) => ({
					...item,
	...
	const selectedDateStr = toLocalDateStr(selectedFullDate);

	const filteredSchedules = Array.isArray(scheduleData)
		? scheduleData.filter((s) => s.localDate === selectedDateStr)
		: [];
					...
					>
						<div className="flex items-center justify-between mb-5">
							...
							<select
								...
								className={`text-sm rounded-lg px-3 py-1.5 outline-none border ${darkMode
						...
						<div
							className={`grid grid-cols-4 font-semibold text-sm pb-3 border-b ${darkMode
						...
						<div className="divide-y divide-slate-700/30 mt-2">
							{allMembers.length === 0 ? (
								...
								filteredMembers.map((member) => (
									<div
										key={member.id}
										className={`grid grid-cols-4 items-center py-3 rounded-xl transition-all hover:scale-102 cursor-pointer ${darkMode
									...
									>
										<div className="flex items-center gap-3">
											...
											<div>
												<h4
													className={`font-semibold capitalize ${darkMode ? "text-white" : "text-slate-900"
														}`}
												>
													{member.name}
												</h4>
										...
										<div
											className={`flex items-center justify-start px-4 py-1.5 rounded-full font-semibold w-fit ${darkMode
										...
										<div
											className={`text-sm font-bold px-4 py-1.5 rounded-full text-right ml-auto w-fit ${darkMode
