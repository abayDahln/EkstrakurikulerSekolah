                ...
                user = await _context.Users
                    ...
                    .Select(u => new
                    {
                        ...
                        ActivityStats = new
                        {
                            TotalAttendances = _context.Attendances
                                .Count(a => _context.Members
                                    .Any(m => m.Id == a.MemberId && m.UserId == id)),
                            TotalReports = _context.ActivityReports
                                .Count(r => _context.Members
                                    .Any(m => m.Id == r.MemberId && m.UserId == id)),
                            JoinedExtracurriculars = u.Members.Count(m => m.Status == "active"),
                            TotalPoints = u.Members
                                .SelectMany(m => m.Point)
                                .Sum(p => p.Points)
            ...
            else 
            {
                user = await _context.Users
                    ...
                    .Select(u => new
                    {
                        ...
                        ActivityStats = new
                        {
                            TotalManagedExtracurriculars = _context.Extracurriculars.Count(e => e.PembinaId == id),
                            TotalSchedulesCreated = _context.Schedules.Count(s => s.Extracurricular.PembinaId == id),
                            UpcomingSchedules = _context.Schedules
                                .Count(s => s.Extracurricular.PembinaId == id && s.ScheduleDate > DateTime.Now),
                            TotalStudents = _context.Members
                                .Count(m => m.Status == "active" && m.Extracurricular.PembinaId == id)
                        }
                    })
                    .FirstOrDefaultAsync();
            }
