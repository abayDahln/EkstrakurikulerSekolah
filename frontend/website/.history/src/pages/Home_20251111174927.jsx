...
const SkeletonEkskulItem = ({ darkMode }) => {
	...
	return (
		<div className="w-full h-64 flex items-end justify-around">
			{/* Contoh 5 batang skeleton */}
			{Array.from({ length: 5 }).map((_, i) => (
				<div key={i} className="flex flex-col items-center space-y-2">
					{/* Batang Skeleton */}
					<div
						className={`w-8 rounded-t-lg animate-pulse ${skeletonColor}`}
						style={{
							height: `${Math.random() * 80 + 40}px`, // tinggi acak agar natural
						}}
					></div>
					{/* Label Skeleton */}
					<div
						className={`w-12 h-3 rounded-md animate-pulse ${skeletonColor}`}
					></div>
				</div>
			))}
		</div>
...
const Home = ({ darkMode }) => {
  ...
  try {
    ...
    console.log("Server error, setting server down state");
...
const maxMember = data.length > 0 ? Math.max(...data.map((d) => d.anggota)) : 0;
