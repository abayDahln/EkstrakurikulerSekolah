...
const AnimatedScheduleCard = ({ schedule, darkMode, index, hasAnimated, onCardClick, API_URL }) => {
	return (
		<motion.div
			...
			>
				...
				<div className="space-y-2 flex-shrink-0">
					...
					>
						...
						<span className="truncate">{schedule.extracurricular.pembina.name}</span>
					</div>
				</div>
			</div>
		</motion.div>
	...
	useEffect(() => {
		...
		const fetchData = async () => {
			try {
				...
				const ekskulResponse = await fetch(`${API_URL}/api/pembina/my-extracurricular`, {
					headers: { Authorization: `Bearer ${token}` },
				});
			...
			<div className="p-8">
				<div className="max-w-7xl mx-auto">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
						...
						) : showErrorState ? ( //WebkitBoxOrient
							// Show error state
