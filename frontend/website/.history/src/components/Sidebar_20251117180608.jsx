...
export default function Sidebar({ darkMode, initialMenu = 1 }) {
	...
	useEffect(() => {
		...
		if (initialMenu) {
			...
			return; // 🔥 hentikan agar initialMenu benar-benar jadi prioritas
		}

		if (path === "/home" || path === "/") setActiveMenu(1);
		else if (path === "/ekskuls" || path.startsWith("/ekskul/")) setActiveMenu(2);
		else if (path === "/jadwal") setActiveMenu(3);
		else if (path === "/profile") setActiveMenu(4);
		else if (path === "/laporan") setActiveMenu(5);
	}, [location.pathname, initialMenu]);
