...
import { FiUser, FiMail, FiAward, FiCalendar, FiUsers, FiTrendingUp, FiArrowLeft, FiTarget, FiCheckCircle, FiRefreshCw } from "react-icons/fi";
...
const UserProfile = ({ darkMode, userId = 14 }) => {
  ...
  // Fungsi untuk fetch data dengan cache busting
  const fetchProfileData = async (isRefresh = false) => {
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/profile/${userId}?t=${timestamp}`, { 
          headers: { 
      ...
      if (result.status === 200 && result.data) {
        ...
        console.log('Profile data updated:', result.data);
      } else {
  ...
  // Fetch data saat component mount dan ketika userId berubah
  useEffect(() => {
    fetchProfileData();
  }, [userId]);
  ...
  // Validasi role dengan default ke siswa jika tidak ada data
  const isPembina = profileData?.role === 'pembina';
  const isSiswa = !isPembina; // Default ke siswa jika bukan pembina

  console.log('Current profile data:', profileData);
  console.log('Is Pembina:', isPembina);
  console.log('Is Siswa:', isSiswa);
                  ...
                  <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
                    <div className="flex justify-between items-center mb-6">
                      ...
                      <span className={`text-sm px-3 py-1 rounded-full ${
                        ...
                        Total: {isPembina ? managedExtracurriculars.length : extracurriculars.length}
                      </span>
                    </div>

                    <div className="space-y-4">
                      {(isPembina ? managedExtracurriculars : extracurriculars)?.length > 0 ? (
                        (isPembina ? managedExtracurriculars : extracurriculars).map((ekskul, index) => (
                          <div
                            key={ekskul.id || index}
