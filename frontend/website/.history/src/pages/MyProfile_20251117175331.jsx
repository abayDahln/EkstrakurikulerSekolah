...
import { FiEdit2, FiSave, FiX, FiCamera, FiUser, FiMail, FiAward, FiCalendar, FiUsers, FiTrendingUp, FiArrowRight } from "react-icons/fi";
import Sidebar from "../components/Sidebar"

// Skeleton Component
const SkeletonProfile = ({ darkMode }) => (
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    {/* Skeleton Profile Card (Kiri) */}
    <div className={`rounded-2xl shadow-lg p-6 flex flex-col justify-between ${darkMode ? 'bg-slate-800' : 'bg-white'} lg:row-span-2`}>
      <div className="text-center">
        <div className="relative inline-block mb-4">
          <div className={`w-32 h-32 rounded-full animate-pulse mx-auto ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        </div>
        
        <div className={`h-8 w-48 rounded animate-pulse mx-auto mb-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        <div className={`h-4 w-64 rounded animate-pulse mx-auto mb-4 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        
        <div className={`h-10 w-32 rounded-full animate-pulse mx-auto ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />

        <div className={`mt-6 pt-6 border-t ${darkMode ? 'border-slate-700' : 'border-slate-200'}`}>
          <div className={`h-3 w-24 rounded animate-pulse mx-auto mb-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
          <div className={`h-5 w-36 rounded animate-pulse mx-auto ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        </div>
      </div>
    </div>

    {/* Skeleton Kanan Atas + Tengah */}
    <div className="lg:col-span-2 flex flex-col gap-6">
      {/* Skeleton Info Card */}
      <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
        <div className="flex items-center justify-between mb-6">
          <div className={`h-7 w-48 rounded animate-pulse ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
          <div className={`h-10 w-24 rounded-lg animate-pulse ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
        </div>

        <div className="space-y-4">
          <div>
            <div className={`h-5 w-32 rounded animate-pulse mb-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
            <div className={`h-12 w-full rounded-xl animate-pulse ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
          </div>
          <div>
            <div className={`h-5 w-20 rounded animate-pulse mb-2 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
            <div className={`h-12 w-full rounded-xl animate-pulse ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />
          </div>
        </div>
      </div>
      ...
      <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
        <div className={`h-7 w-48 rounded animate-pulse mb-6 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />

        <div className="grid grid-cols-2 gap-4">
          {[...Array(4)].map((_, index) => (
            <div key={index} className={`p-4 rounded-xl ${darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
              <div className="flex items-center gap-2 mb-2">
                <div className={`w-10 h-10 rounded-lg animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-200'}`} />
              </div>
              <div className={`h-8 w-16 rounded animate-pulse mb-1 ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              <div className={`h-4 w-20 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
            </div>
          ))}
        </div>
      </div>
    ...
    {/* Skeleton Kanan Bawah - Full Width */}
    <div className={`lg:col-span-3 rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
      <div className={`h-7 w-64 rounded animate-pulse mb-6 ${darkMode ? 'bg-slate-700' : 'bg-slate-200'}`} />

      <div className="space-y-4">
        {[...Array(2)].map((_, index) => (
          <div key={index} className={`flex items-center gap-4 p-4 rounded-xl ${darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
            <div className={`w-16 h-16 rounded-xl animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-200'}`} />
            <div className="flex-1 space-y-2">
              <div className="flex items-center justify-between">
                <div className={`h-6 w-32 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
                <div className={`w-5 h-5 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              </div>
              <div className={`h-4 w-48 rounded animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              <div className="flex gap-3">
                <div className={`h-6 w-24 rounded-full animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
                <div className={`h-6 w-20 rounded-full animate-pulse ${darkMode ? 'bg-slate-600' : 'bg-slate-300'}`} />
              </div>
            </div>
          </div>
        ))}
...
// Main My Profile Component (Pembina Only)
const MyProfile = ({ darkMode }) => {
  ...
  // Hitung total siswa dari semua ekskul
  const calculateTotalStudents = () => {
    if (!profileData?.managedExtracurriculars) return 0;
    return profileData.managedExtracurriculars.reduce((total, ekskul) => total + (ekskul.totalMembers || 0), 0);
  };
  ...
  return (
    <div className={`min-h-screen transition-colors duration-300 ${
      ...
      <Sidebar 
        darkMode={darkMode} initialMenu={activeMenu} />
                        ...
                        {isEditingInfo ? (
                          <input 
                            ...
                            className={`w-full p-3 rounded-xl border-2 transition-all ${
                              ...
                              darkMode ? 'focus:ring-sky-900/50 focus:border-sky-500' : 'focus:ring-sky-100 focus:border-sky-500'
                            }`}
                          />
                        ) : (
      ...
      <div className={`rounded-2xl shadow-lg p-6 ${darkMode ? 'bg-slate-800' : 'bg-white'}`}>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      <div className={`p-4 rounded-xl ${darkMode ? 'bg-slate-700/50' : 'bg-slate-50'}`}>
                        <div className="flex items-center gap-2 mb-2">
                          <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                            darkMode ? 'bg-orange-900/50 text-orange-300' : 'bg-orange-100 text-orange-600'
                          }`}>
                            <FiCalendar className="text-xl" />
                          </div>
                        </div>
