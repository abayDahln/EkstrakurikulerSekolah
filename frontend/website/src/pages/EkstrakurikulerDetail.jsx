...
const SkeletonDetail = ({ darkMode }) => (
	<div className="space-y-8 w-full">
		<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
			<div
				className={`h-96 rounded-2xl animate-pulse ${darkMode ? "bg-slate-700" : "bg-slate-200"
					}`}
			/>

			<div className="flex flex-col justify-center space-y-4">
				<div
					className={`h-10 w-2/3 rounded animate-pulse ${darkMode ? "bg-slate-700" : "bg-slate-200"
						}`}
				/>
				<div
				...
				/>
				<div
					className={`rounded-xl p-6 mt-4 animate-pulse ${darkMode ? "bg-slate-800" : "bg-white"
						} shadow-lg`}
				>
					<div className="flex items-center gap-4">
						<div
							className={`w-16 h-16 rounded-full ${darkMode ? "bg-slate-700" : "bg-slate-200"
								}`}
						/>
						<div className="flex-1 space-y-2">
							<div
								className={`h-4 w-1/2 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
									}`}
							/>
							<div
								className={`h-3 w-1/3 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
									}`}
							/>
						</div>
					</div>
		...
		<div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
			...
			>
				<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
					<div
						className={`h-6 w-32 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
							} animate-pulse`}
					/>
					<div
						className={`h-8 w-36 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
							} animate-pulse`}
					/>
				</div>
				...
				<div className="space-y-4">
					{Array(4)
						...
						.map((_, i) => (
							<div
								key={i}
								className={`rounded-xl p-4 flex items-center gap-4 animate-pulse ${darkMode ? "bg-slate-700/50" : "bg-slate-100"
									}`}
							>
								<div
									className={`w-14 h-14 rounded-full ${darkMode ? "bg-slate-600" : "bg-slate-300"
										}`}
								/>
								<div className="flex-1 space-y-2">
									<div
										className={`h-4 w-1/2 rounded ${darkMode ? "bg-slate-600" : "bg-slate-300"
											}`}
									/>
									<div
										className={`h-3 w-1/3 rounded ${darkMode ? "bg-slate-600" : "bg-slate-300"
											}`}
									/>
								</div>
							</div>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
					{Array(4)
						...
						.map((_, i) => (
							<div
								key={i}
								className={`rounded-xl p-6 shadow-lg animate-pulse ${darkMode ? "bg-slate-800" : "bg-white"
									}`}
							>
								<div
									className={`w-12 h-12 rounded-xl mb-4 ${darkMode ? "bg-slate-700" : "bg-slate-200"
										}`}
								/>
								<div
									className={`h-6 w-1/3 mb-2 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
										}`}
								/>
								<div
									className={`h-3 w-1/2 rounded ${darkMode ? "bg-slate-700" : "bg-slate-200"
										}`}
								/>
							</div>
						))}
				...
				>
					<div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
						...
						<div className="flex flex-col justify-center">
							<h1
								className={`text-4xl font-extrabold mb-4 ${darkMode ? "text-white" : "text-slate-900"
									}`}
							>
								{ekskulDetail.name}
							</h1>
							...
							{ekskulDetail.pembinas &&
								ekskulDetail.pembinas.length > 0 && (
									<div
										className={`rounded-xl p-6 mb-6 ${darkMode ? "bg-slate-800" : "bg-white"
											} shadow-lg cursor-pointer hover:scale-105 transition-transform`}
										onClick={() =>
											handleProfileClick(ekskulDetail.pembinas[0].id)
										}
									>
										<p
											className={`text-sm font-semibold mb-3 ${darkMode ? "text-slate-400" : "text-slate-500"
												}`}
										>
											Pembina
										</p>
										<div className="flex items-center gap-4">
											<img
												src={`${API_URL}/${ekskulDetail.pembinas[0].profile || ""
													}`}
					...
					<div className="grid grid-cols-1 xl:grid-cols-2 gap-8">
						...
						>
							<div className="flex flex-col sm:flex-row sm:items-center justify-between mb-6 gap-4">
								...
								<select
									...
									className={`px-4 py-2 rounded-lg border transition-colors pl-3 ${darkMode
							...
							{sortedMembers.length > 0 ? (
								<div className="space-y-4">
									{sortedMembers.map((member) => (
										<div
											key={member.id}
											className={`rounded-xl p-4 transition-all hover:scale-105 cursor-pointer ${darkMode
											...
											<div className="flex items-center gap-4">
												...
												<div className="flex-1 min-w-0">
													<div className="flex items-center justify-between mb-1">
														<h4
															className={`font-bold truncate ${darkMode ? "text-white" : "text-slate-900"
																}`}
														>
															{member.name}
														</h4>
						...
						<div className="space-y-6">
							<div className="grid grid-cols-1 md:grid-cols-2 gap-4">
								<div
									className={`rounded-xl p-6 ${darkMode ? "bg-slate-800" : "bg-white"
										} shadow-lg`}
								>
									<div
										className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${darkMode ? "bg-sky-900/50" : "bg-sky-100"
											}`}
									>
										<FaUsers
											className={`text-xl ${darkMode ? "text-sky-400" : "text-sky-600"
												}`}
										/>
									</div>
									<h3
								...
								>
									<div
										className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${darkMode ? "bg-purple-900/50" : "bg-purple-100"
											}`}
									>
										<FaStar
								...
								>
									<div
										className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${darkMode ? "bg-green-900/50" : "bg-green-100"
											}`}
									>
										<FaChartBar
								...
								>
									<div
										className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${darkMode ? "bg-orange-900/50" : "bg-orange-100"
											}`}
									>
										<FaTrophy
											className={`text-xl ${darkMode ? "text-orange-400" : "text-orange-600"
												}`}
										/>
									</div>
									<h3
